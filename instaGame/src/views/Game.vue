...
<script>
	...
	let checkerboardTask, circleTask, crossTask, turn;
	let	message = 'Inactive Game Room', leaveButtonVisible = false;
	export default {
	data() {
		return {
			message: message,
	...
	methods: {
		...
		this.$nextTick(() => {
			...
			socket.emit('joinroom', { player_id: this.player_id, room_id: this.room_id });
		...
		socket.on('turnstart_message', (data) => {
			...
			message = data.message + ', make a move.';
			} else {
		...
		checkerboardTask.onSuccess = function (task) {
			...
			const cellSize =
			(Math.max(boundingBox.maximumWorld.x - boundingBox.minimumWorld.x, boundingBox.maximumWorld.z - boundingBox.minimumWorld.z) /
				3.0) *
			0.8;
			for (let i = 0; i < 3; i++) {
			for (let j = 0; j < 3; j++) {
		...
		assetsManager.onFinish = function (tasks) {
			canvas.addEventListener('click', (event) => {
				...
				if (pickResult.hit && pickResult.pickedMesh && pickResult.pickedMesh.name === 'Checkerboard') {
					const closestCell = this.findClosestCell(pickResult.pickedPoint);
					if (!closestCell.occupied) this.makeMove(closestCell.index);
				}
			});
